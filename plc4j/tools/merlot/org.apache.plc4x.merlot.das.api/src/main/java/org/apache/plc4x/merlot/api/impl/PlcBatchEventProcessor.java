 ...
 * from a {@link RingBuffer} and delegating the available 
 * events to an {@link EventHandler}.
 * 
 * Simple adaptation of BatchEventProcessor<T> from the Disruptor project to 
 * comply with read/write exclusion. 
 *
 * @param <T> event implementation storing the data for sharing during exchange or parallel coordination of an event.
 */
public final class PlcBatchEventProcessor<T>
        implements EventProcessor
{
    private static final int IDLE = 0;
    private static final int HALTED = IDLE + 1;
    private static final int RUNNING = HALTED + 1;
    private static final int PAUSED = RUNNING + 1;    
    ...
    private void processEvents()
    {
        ...
        while (true)
        {
            if (running.get() == PAUSED) continue;
            ...
            try
            {
                try
                {
                    ...
                    //TODO: How disable the compiler catch error?
                    if (false) throw new RewindableException(null);                    
                }
                //No rewing Handler for this Object
