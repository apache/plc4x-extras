...
public class B2MMLRepositoryImpl implements S88Repository {
    ...
    private S88Element mapToApi(EquipmentType xml) {
        S88Element element = new S88ElementImpl();
        ...
        element.getS88Identity().setId(id);
        element.getS88Identity().setLevel(mapLevelFromB2MML(lvlTxt));
        
        if (xml.sizeOfDescriptionArray() > 0) {
            element.getS88PropertyBag().setDescription(xml.getDescriptionArray(0).getStringValue());
        }
    ...
    private void mapToXml(S88Element element, EquipmentType equipment) {
        equipment.addNewID().setStringValue(element.getS88Identity().getId());
        S88Level level = element.getS88Identity().getLevel();
        if (level != null && !level.isEmpty()) {
            equipment.addNewEquipmentLevel().setStringValue(mapLevelToB2MML(level));
        }
        if (element.getS88PropertyBag().getDescription() != null) {
            equipment.addNewDescription().setStringValue(element.getS88PropertyBag().getDescription());
        }

        for (var entry : element.getS88PropertyBag().getProperties().entrySet()) {
        ...
        for (S88Element childApi : element.getS88Hierarchy().getChildren()) {
    ...
    private void buildHierarchy(EquipmentType[] equipment, S88Element element) {
        for (EquipmentType childXml : equipment) {
            element.getS88Hierarchy().addChild(mapToApi(childXml));
        }
    }
        ...
        for (EquipmentPropertyType prop : properties) {
            ...
            if (prop.sizeOfValueArray() > 0) {
                ...
                if (val.getValueString() != null) {
                    element.getS88PropertyBag().setProperty(propId, val.getValueString().getStringValue());
                }
            }
        }
    }
